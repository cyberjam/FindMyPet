...
def view_init():
    gmaps_key = "AIzaSyAAOAcO79nNaGUwg7NLCUYQNisqW-yiNGo"
...
def test(center,gmaps,view):
    geocode_result = gmaps.geocode("대전광역시 한밭대로1114번길 34")
    location = list(geocode_result[0]['geometry']['location'].values())
    image_tag = f'<img src="https://www.daejeon.go.kr/FileUpload/ANI/202105/20210511051024820.jpg", height="150" width=auto>'
    iframe = folium.IFrame(image_tag, width=180, height=250)
    popup = folium.Popup(iframe, max_width=650)

    folium.Marker(location, popup=popup, tooltip='test' , icon=folium.Icon(icon='cloud', color = 'red')).add_to(view)
    folium.Circle(location, radius=1000, color='#FCAE60', fill_color='#FCAE60').add_to(view)

    geocode_result = gmaps.geocode("대전광역시 한밭대로1114번길 10")
    location = list(geocode_result[0]['geometry']['location'].values())
    image_tag = f'<img src="https://www.daejeon.go.kr/FileUpload/ANI/202105/20210511051024820.jpg", height="150" width=auto>'
    iframe = folium.IFrame(image_tag, width=180, height=250)
    popup = folium.Popup(iframe, max_width=650)
...
def index():
    ...
    # test(center,gmaps,view)
    # image_tag = f'<img src="https://www.daejeon.go.kr/FileUpload/ANI/202105/20210511051024820.jpg", height="150" width=auto>'
    # iframe = folium.IFrame(image_tag, width=180, height=250)
    # popup = folium.Popup(iframe, max_width=650)

    # folium.Marker(center, popup=popup, tooltip='test' , icon=folium.Icon(icon='cloud', color = 'red')).add_to(view)
    # folium.Circle(center, radius=1000, color='#FCAE60', fill_color='#FCAE60').add_to(view)
    
    # # view, gmaps = view_init()
